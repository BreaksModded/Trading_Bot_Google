...
def test_price_shock_detected_within_one_interval(risk_manager):
    """Inject a price shock into the RiskManager data queue. Assert caught within 5 seconds."""
    ...
    # It might cache the old safe result!
    with patch('time.monotonic', return_value=2.0):
        with patch('time.time', return_value=3600.0): # mock time for deque retention if tested purely
            risk_manager.register_price(now=datetime.now(UTC), price=53000.0)
            safe_res = risk_manager.evaluate(equity=1000.0)
            # Still safe because cache interval hasn't expired
            assert safe_res.allow_trading is True
    
    # But after 5 seconds expire (at t=6.0), the shock is detected!
    with patch('time.monotonic', return_value=6.0):
        with patch('time.time', return_value=3600.0):
            ...
            assert shock_res.emergency_stop is True
...
def test_cached_result_is_per_instance_not_class_variable():
    ...
    rm_1 = RiskManager(max_drawdown_pct=0.25, max_daily_loss_pct=0.10, max_hourly_move_pct=0.05)
    rm_2 = RiskManager(max_drawdown_pct=0.25, max_daily_loss_pct=0.10, max_hourly_move_pct=0.05)
    ...
    with patch('time.monotonic', return_value=6.0):
        with patch('time.time', return_value=3600.0):
            ...
            res_1 = rm_1.evaluate(equity=1000) # Shock 6%
            res_2 = rm_2.evaluate(equity=1000) # Flat
            
            assert res_1.emergency_stop is True
            assert res_2.emergency_stop is False
